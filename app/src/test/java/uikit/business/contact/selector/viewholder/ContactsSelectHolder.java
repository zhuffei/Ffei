...
import com.netease.nim.uikit.common.util.media.AsyncImageLoader;
...
public class ContactsSelectHolder extends AbsContactViewHolder<ContactItem> {
    ...
    public void refresh(ContactDataAdapter adapter, int position, ContactItem item) {
        ...
        Log.d("aaaaaaaaaa",contact.getContactType()+"");
        if (contact.getContactType() == IContact.Type.Friend || contact.getContactType() == IContact.Type.Msg) {
            this.nickname.setText(contact.getDisplayName());
            //加的代码
            String avator = NimUIKit.getUserInfoProvider().getUserInfo(contact.getContactId()).getAvatar();
            Log.d("aaaaaaaaaa",avator);
            asyncImageLoader.asyncloadImage(image, UrlTool.AVATOR+avator);
